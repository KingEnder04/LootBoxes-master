...
import lootboxes.LBContent.LootTables.LootBoxTierFourLootTable;
...
public class LootBoxTierFour extends MultiTextureMatItem {
    public LootBoxTierFour() {
    ...
    public Supplier<ContainerActionResult> getInventoryRightClickAction(Container container, InventoryItem item, int slotIndex, ContainerSlot slot) {
        return () -> {
            if (container.getClient().isServer()) {
                ...
                LootBoxTierFourLootTable.lootBox.addItems(itemList, GameRandom.globalRandom, 1.0F, new Object[]{container.getClient()});

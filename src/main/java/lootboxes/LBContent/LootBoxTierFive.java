...
import lootboxes.LBContent.LootTables.LootBoxTierFiveLootTable;
...
public class LootBoxTierFive extends MultiTextureMatItem {
    public LootBoxTierFive() {
    ...
    public Supplier<ContainerActionResult> getInventoryRightClickAction(Container container, InventoryItem item, int slotIndex, ContainerSlot slot) {
        return () -> {
            if (container.getClient().isServer()) {
                ...
                LootBoxPresets.lootBox5.addItems(itemList, GameRandom.globalRandom, 1.0F, new Object[]{container.getClient()});
